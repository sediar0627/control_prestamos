function trimCanvas(canvas) {
    ...
    newCanvasContext.drawImage(canvas, 0, 0, imgWidth, imgHeight)

    let imgData = newCanvasContext.getImageData(0, 0, imgWidth, imgHeight).data
    ...
    let trimmedData = newCanvasContext.getImageData(cropLeft, cropTop, cropXDiff,
        cropYDiff)

    // set the trimmed width and height
    newCanvas.width = cropXDiff
    newCanvas.height = cropYDiff
    // clear the canvas
    newCanvasContext.clearRect(0, 0, cropXDiff, cropYDiff)
    // place the trimmed data into the cleared canvas to create
    // a new, trimmed canvas
    newCanvasContext.putImageData(trimmedData, 0, 0)
    return newCanvas // for chaining
}
